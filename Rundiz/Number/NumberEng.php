...
class NumberEng
{
    ...
    public function convertNumber($num)
    {
        ...
        if ($num{0} == '0') {
            ...
            for ($z = 0; $z < count($groups2); $z++) {
                if ($groups2[$z] != '') {
                    ...
                    $output .= $groups2[$z] . $this->convertGroup(11 - $z) . ($z < 11 && !array_search('', array_slice($groups2, $z + 1, -1)) && $groups2[11] != '' && $groups[11]{0} == '0' ? ' ' : ' ');
                }
            }// endfor;
        ...
        if ($dec > 0) {
            $stang = $this->convertTwoDigit($dec{0}, $dec{1});
            if ($stang != '') {
              $output .= ' baht and '. $stang. ' Satang';
            }
            //$output .= ' baht and';
            //for ($i = 0; $i < strlen($dec); $i++) {
            //    $output .= ' ' . $this->convertDigit($dec{$i});
            //}// endfor;
            //$output .= ' ' . $this->convertTwoDigit($dec{0}, $dec{1});
            //$output .= ' satang';
