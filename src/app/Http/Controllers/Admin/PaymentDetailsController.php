...
class PaymentDetailsController extends AdminController
{
	...
	public function save(Request $request)
	{
			...
			if ($request->docuno_id > 0 && $request->listno > 0 && $request->goodprice2 <> 'null' && $request->goodqty2 > 0 && $request->goodcode <> "" && $request->goodnameeng1 <> "" && $request->rematotalamnt <> 'null') {
				// OK
			} else {
			...
			if ($data_ok) {
					...
					if ($hasMaster->count() == 0){
						$response = array ('status' => 'error', 'msg' => 'Payment not found.');
					} else {
