    ...
    <div class="box-body">
        <div>
            ...
            <input type="text" class="form-control" id="{{ $titleId }}" name="{{ $titleId }}"
                   value="{{ Helper::getValue($content, 'title', $errors, $locale) }}" required/>
        </div>
        <br/>
        <div>
            <label for="{{ $detailId }}">{{ trans('admin.contents-detail', [], $transDomain, $locale) }}
                <span class="text-danger">*</span></label>
            <textarea class="hidden" id="{{ $detailId }}"
                      name="{{ $detailId }}">{{ Helper::getValue($content, 'detail', $errors, $locale) }}</textarea>
        </div>
    </div>
</div>
