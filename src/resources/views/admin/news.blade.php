...
@section('content')
    <div class="box box-solid">
        <div class="box-header with-border">
            <a class="btn btn-success" href="{{ Helper::url('admin/news/create') }}">
        ...
        <div class="box-body">
            <table id="table-result" class="table table-bordered table-hover">
                ...
                @foreach($news as $n)
                    <tr id="row-{{ $n->id }}">
                        <td>{{ $n->translate('en')->title }}</td>
                        <td>{{ $n->translate('th')->title }}</td>
                        <td>{{ Helper::datetime($n->news_date) }}</td>
                        <td>{{ $n->sort }}</td>
                        <td align="center">
                            @if($n->active)
                        ...
                        <td align="center">
                            <a class="btn btn-sm btn-warning"
                               href="{{ Helper::url('admin/news/' . $n->id . '/edit') }}">{{ trans('admin.contents-edit') }}</a>
                        </td>
                        <td align="center">
...
@section('script')
    <script type="text/javascript">
        $(function () {
            ...
            @include('admin.view.view-delete-js', ['url' => 'admin/news'])
        });
    </script>
@endsection
