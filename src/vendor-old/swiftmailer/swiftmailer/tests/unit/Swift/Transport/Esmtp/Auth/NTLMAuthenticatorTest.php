...
class Swift_Transport_Esmtp_Auth_NTLMAuthenticatorTest extends \SwiftMailerTestCase
{
    ...
    public function test_keyword_is_ntlm()
    {
        $login = $this->_getAuthenticator();
        $this->assertEquals('NTLM', $login->getAuthKeyword());
    }

    public function test_message1_generator()
    {
        $login = $this->_getAuthenticator();
    ...
    public function test_l_mv1_generator()
    {
        $password = 'test1234';
        ...
        $login = $this->_getAuthenticator();
        $lmv1Result = $this->_invokePrivateMethod('createLMPassword', $login, [$password, $this->hex2bin($challenge)]);

        $this->assertEquals($lmv1, bin2hex($lmv1Result), '%s: The keys should be the same cause we use the same values to generate them.');
    }

    public function test_l_mv2_generator()
    {
        $username = 'user';
        $password = 'SecREt01';
        $domain = 'DOMAIN';
        ...
        $login = $this->_getAuthenticator();
        $lmv2Result = $this->_invokePrivateMethod('createLMv2Password', $login, [$password, $username, $domain, $this->hex2bin($challenge), $this->hex2bin('ffffff0011223344')]);

        $this->assertEquals($lmv2, bin2hex($lmv2Result), '%s: The keys should be the same cause we use the same values to generate them.');
    }

    public function test_message3v1_generator()
    {
        $username = 'test';
        $domain = 'TESTNT';
        $workstation = 'MEMBER';
        ...
        $login = $this->_getAuthenticator();
        $message3 = $this->_invokePrivateMethod('createMessage3', $login, [$domain, $username, $workstation, $this->hex2bin($lmResponse), $this->hex2bin($ntlmResponse)]);
    ...
    public function test_message3v2_generator()
    {
        $username = 'test';
        $domain = 'TESTNT';
        $workstation = 'MEMBER';
    ...
    public function test_get_domain_and_username()
    {
        $username = "DOMAIN\user";

        $login = $this->_getAuthenticator();
        [$domain, $user] = $this->_invokePrivateMethod('getDomainAndUsername', $login, [$username]);
    ...
    public function test_get_domain_and_username_with_extension()
    {
    ...
    public function test_get_domain_and_username_with_at_symbol()
    {
    ...
    public function test_get_domain_and_username_with_at_symbol_and_extension()
    {
    ...
    public function test_get_domain_and_username_without_domain()
    {
        $username = 'user';
    ...
    private function _invokePrivateMethod($method, $instance, array $args = [])
    {
