...
class Standard extends PrettyPrinterAbstract
{
    ...
    public function pExpr_Closure(Expr\Closure $node)
    {
        return ($node->static ? 'static ' : '')
             ...
             .($node->returnType !== null ? ' : '.$this->pType($node->returnType) : '')
    ...
    public function pExpr_Ternary(Expr\Ternary $node)
    {
        ...
        return $this->pInfixOp('Expr_Ternary',
            $node->cond, ' ?'.($node->if !== null ? ' '.$this->p($node->if).' ' : '').': ', $node->else
        );
    }
    ...
    public function pStmt_TraitUseAdaptation_Alias(Stmt\TraitUseAdaptation\Alias $node)
    {
        return ($node->trait !== null ? $this->p($node->trait).'::' : '')
             .$node->method.' as'
             .($node->newModifier !== null ? ' '.rtrim($this->pModifiers($node->newModifier), ' ') : '')
    ...
    {
        return ($node->type === 0 ? 'var ' : $this->pModifiers($node->type)).$this->pCommaSeparated($node->props).';';
    }
    ...
    public function pStmt_Declare(Stmt\Declare_ $node)
    {
        return 'declare ('.$this->pCommaSeparated($node->declares).')'
             .($node->stmts !== null ? ' {'.$this->pStmts($node->stmts)."\n".'}' : ';');
    }
    ...
    public function pStmt_If(Stmt\If_ $node)
    {
        return 'if ('.$this->p($node->cond).') {'
             ...
             .($node->else !== null ? $this->p($node->else) : '');
    }
