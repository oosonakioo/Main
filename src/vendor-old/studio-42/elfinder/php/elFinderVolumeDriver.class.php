...
abstract class elFinderVolumeDriver
{
    ...
    public function search($q, $mimes, $hash = null)
    {
        ...
        if ($this->options['searchExDirReg']) {
            if (preg_match($this->options['searchExDirReg'], '') === false) {
    ...
    protected function checkExtractItems($path, $checks = null)
    {
        ...
        if (is_dir($path)) {
            ...
            if ($chkSymlink && is_link($path)) {
            ...
            } elseif ($chkName && ! $this->nameAccepted($name)) {
                unlink($path);
                $res['names'][] = $path;
                $res['rmNames'][] = $name;
                ...
                $res['writables'][] = $path;
                $res['rmNames'][] = $name;
            } elseif ($chkMime && ($mimeByName = elFinderVolumeDriver::mimetypeInternalDetect($name)) && $mimeByName !== 'unknown' && ! $this->allowPutMime($mimeByName)) {
                unlink($path);
                $res['mimes'][] = $path;
                $res['rmNames'][] = $name;
            } else {
