    ...
    { // mPDF 6
        ...
        switch ($tag) {
            ...
            case 'TOCPAGEBREAK': // custom-tag - set Marker for insertion later of ToC AND adds PAGEBREAK
                ...
                [$isbreak, $toc_id] = $this->mpdf->tocontents->openTagTOCPAGEBREAK($attr);
                if ($isbreak) {
                    break;
                }
            ...
            case 'FORMFEED': // custom-tag
                ...
                if (isset($attr['SHEET-SIZE'])) {
                    ...
                    if (count($prop) == 2) {
                        $newformat = [$this->mpdf->ConvertSize($prop[0]), $this->mpdf->ConvertSize($prop[1])];
                    } else {
            ...
            case 'MAIN':
                ...
                if (isset($properties['FLOAT']) && strtoupper($properties['FLOAT']) == 'RIGHT' && ! $this->mpdf->ColActive) {
                    ...
                    [$l_exists, $r_exists, $l_max, $r_max, $l_width, $r_width] = $this->mpdf->GetFloatDivInfo($this->mpdf->blklvl - 1);
                    ...
                    if (($setwidth + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) > $maxw || ($maxw - ($currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr)) < (2 * $this->mpdf->GetCharWidth('W', false))) {
                        ...
                        } elseif ($r_max < $l_max && ($setwidth + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) <= ($container_w - $l_width) && (($container_w - $l_width) - ($currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr)) > (2 * $this->mpdf->GetCharWidth('W', false))) {
                    ...
                    if (($setwidth + $currblk['margin_left'] + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) > $maxw || ($maxw - ($currblk['margin_right'] + $currblk['margin_left'] + $bdl + $pdl + $bdr + $pdr)) < (2 * $this->mpdf->GetCharWidth('W', false))) {
                        ...
                        if ($l_max < $r_max && ($setwidth + $currblk['margin_left'] + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) <= ($container_w - $r_width) && (($container_w - $r_width) - ($currblk['margin_right'] + $currblk['margin_left'] + $bdl + $pdl + $bdr + $pdr)) > (2 * $this->mpdf->GetCharWidth('W', false))) {
                            $this->mpdf->ClearFloats('LEFT', $this->mpdf->blklvl - 1);
                        } elseif ($r_max < $l_max && ($setwidth + $currblk['margin_left'] + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) <= ($container_w - $l_width) && (($container_w - $l_width) - ($currblk['margin_right'] + $currblk['margin_left'] + $bdl + $pdl + $bdr + $pdr)) > (2 * $this->mpdf->GetCharWidth('W', false))) {
                            $this->mpdf->ClearFloats('RIGHT', $this->mpdf->blklvl - 1);
                        } else {
                            $this->mpdf->ClearFloats('BOTH', $this->mpdf->blklvl - 1);
                        }
            ...
            case 'BARCODE':
                ...
                if (isset($attr['CODE']) && $attr['CODE']) {
                    ...
                    } elseif ($objattr['btype'] == 'IMB' || $objattr['btype'] == 'RM4SCC' || $objattr['btype'] == 'KIX' || $objattr['btype'] == 'POSTNET' || $objattr['btype'] == 'PLANET') {
                    ...
                    } elseif (in_array($objattr['btype'], ['C128A', 'C128B', 'C128C', 'EAN128A', 'EAN128B', 'EAN128C', 'C39', 'C39+', 'C39E', 'C39E+', 'S25', 'S25+', 'I25', 'I25+', 'I25B', 'I25B+', 'C93', 'MSI', 'MSI+', 'CODABAR', 'CODE11'])) {
            ...
            case 'TABLE': // TABLE-BEGIN
                ...
                if (isset($properties['LANG']) && $properties['LANG']) {
                    if ($this->mpdf->autoLangToFont && ! $this->mpdf->usingCoreFont) {
                        if ($properties['LANG'] != $this->mpdf->default_lang && $properties['LANG'] != 'UTF-8') {
                            [$coreSuitable, $mpdf_pdf_unifont] = GetLangOpts($properties['LANG'], $this->mpdf->useAdobeCJK, $this->mpdf->fontdata);
                            if ($mpdf_pdf_unifont) {
                ...
                if ($this->mpdf->tableLevel == 1 && $this->mpdf->useGraphs) {
                    ...
                    $this->mpdf->graphs[$this->mpdf->currentGraphId] = [];
                }
                // ++++++++++++++++++++++++++++
            ...
            case 'TD':
                ...
                if ($this->mpdf->col + 1 > $this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['nc']) {
                ...
                if ($this->mpdf->col + $cs > $this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['nc']) {
    ...
    { // mPDF 6
        ...
        if ($tag == 'TABLE') { // TABLE-END (
            ...
            if (isset($this->mpdf->table[1][1]['overflow']) && $this->mpdf->table[1][1]['overflow'] == 'visible') {
                ...
                [$check, $tablemiw] = $this->mpdf->_tableColumnWidth($this->mpdf->table[1][1], true);
            ...
            for ($lvl = 2; $lvl <= $this->mpdf->innermostTableLevel; $lvl++) {
                for ($nid = 1; $nid <= $this->mpdf->tbctr[$lvl]; $nid++) {
                    ...
                    [$parentrow, $parentcol, $parentnid] = $this->mpdf->table[$lvl][$nid]['nestedpos'];
                    ...
                    if (! $this->mpdf->simpleTables) {
                        if ($this->mpdf->packTableData) {
                            [$bt, $br, $bb, $bl] = $this->mpdf->_getBorderWidths($c['borderbin']);
                        } else {
            ...
            for ($lvl = $this->mpdf->innermostTableLevel; $lvl > 0; $lvl--) {
                for ($nid = 1; $nid <= $this->mpdf->tbctr[$lvl]; $nid++) {
                    [$tableheight, $maxrowheight, $fullpage, $remainingpage, $maxfirstrowheight] = $this->mpdf->_tableHeight($this->mpdf->table[$lvl][$nid]);
                }
            }
            if ($this->mpdf->progressBar) {
            ...
            if ($this->mpdf->table[1][1]['overflow'] == 'visible') {
                ...
                $finished = false;
                while (! $finished) {
                    [$finished, $r, $c, $p, $y, $y0] = $this->mpdf->_tableWrite($this->mpdf->table[1][1], true, $r, $c, $p, $y);
                    if (! $finished) {
                        $this->mpdf->AddPage($this->mpdf->CurOrientation);
            ...
            $this->mpdf->cssmgr->tablecascadeCSS = [];
