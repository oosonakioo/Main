...
$excl = ['HTML-CSS', 'DIRECTW', 'TABLES', 'IMAGES-CORE',
...
if (! isset($_POST['generate']) || $_POST['generate'] != 'generate') {
    ...
    if (! file_exists('mpdf_source.php')) {
        exit('ERROR - Could not find mpdf_source.php file in current directory. Please rename mpdf.php as mpdf_source.php');
    }
...
if (! count($l)) {
    exit('ERROR - Could not find mpdf_source.php file in current directory');
}
$exclflags = [];
$x = '';
...
if (! $check) {
    exit('ERROR - Could not write to mpdf.php file. Are permissions correctly set?');
}
