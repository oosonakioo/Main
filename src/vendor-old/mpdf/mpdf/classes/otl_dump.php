...
class OTLdump
{
    ...
    public function getMetrics($file, $fontkey, $TTCfontID, $debug, $BMPonly, $kerninfo, $useOTL, $mode)
    {
        // mPDF 5.7.1
        $this->mode = $mode;
    ...
    public function extractInfo($debug = false, $BMPonly = false, $kerninfo = false, $useOTL = 0)
    {
        ...
        if ($useOTL) {
            $this->_getGDEFtables();
            [$this->GSUBScriptLang, $this->GSUBFeatures, $this->GSUBLookups, $this->rtlPUAstr, $this->rtlPUAarr] = $this->_getGSUBtables();
            [$this->GPOSScriptLang, $this->GPOSFeatures, $this->GPOSLookups] = $this->_getGPOStables();
...
?>';
                ...
                elseif ($Lookup[$luli]['Type'] == 3) {
                    ...
                    for ($i = 0; $i < $EntryExitCount; $i++) {
                        ...
                        if ($EntryAnchor != 0) {
                            ...
                            [$x, $y] = $this->_getAnchorTable($EntryAnchor);
                            if ($dir == 'RTL') {
                        ...
                        if ($ExitAnchor != 0) {
                            ...
                            [$x, $y] = $this->_getAnchorTable($ExitAnchor);
                            if ($dir == 'LTR') {
                                if (round($pdfWidth) == round($x * 1000 / $this->mpdf->fonts[$this->fontkey]['desc']['unitsPerEm'])) {
                                    $x = 0;
                                } else {
    ...
    public function _getMarkRecord($offset, $MarkPos)
    {
        ...
        [$x, $y] = $this->_getAnchorTable($MarkAnchor);
