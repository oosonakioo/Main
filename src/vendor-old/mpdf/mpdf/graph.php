...
function print_graph($g, $pgwidth)
{
    ...
    if (! in_array($type, ['bar', 'horiz_bar', 'line', 'radar', 'pie', 'pie3d', 'xy', 'scatter'])) {
        $type = 'bar';
    } // Default=bar
    ...
    if (! in_array($xaxis, ['text', 'lin', 'linear', 'log'])) {
        $xaxis = 'text';
    } // Default=text
    if ($xaxis == 'linear') {
        $xaxis = 'lin';
    ...
    if (! in_array($yaxis, ['lin', 'linear', 'log', 'percent'])) {
        $yaxis = 'lin';
    }   // Default=lin
    if ($yaxis == 'percent') {
        $show_percent = true;
    ...
    // Default sizes
    $defsize = [];
    $defsize['pie'] = ['w' => 600, 'h' => 300];
    $defsize['pie3d'] = ['w' => 600, 'h' => 300];
    $defsize['radar'] = ['w' => 600, 'h' => 300];
    $defsize['line'] = ['w' => 600, 'h' => 400];
    $defsize['xy'] = ['w' => 600, 'h' => 400];
    $defsize['scatter'] = ['w' => 600, 'h' => 400];
    $defsize['bar'] = ['w' => 600, 'h' => 400];
    $defsize['horiz_bar'] = ['w' => 600, 'h' => 500];
    ...
    if (count($data) > 0 && $type) {
        ...
        if ($bandw) {
            $colours = ['snow1', 'black', 'snow4', 'snow3', 'snow2', 'cadetblue4', 'cadetblue3', 'cadetblue1', 'bisque4', 'bisque2', 'beige'];
        } else {
            $colours = ['cyan', 'darkorchid4', 'cadetblue3', 'khaki1', 'darkolivegreen2', 'cadetblue4', 'coral', 'cyan4', 'rosybrown3', 'wheat1'];
        }
        $fills = ['navy', 'orange', 'red', 'yellow', 'purple', 'navy', 'orange', 'red', 'yellow', 'purple'];
        $patterns = [PATTERN_DIAG1, PATTERN_CROSS1, PATTERN_STRIPE1, PATTERN_DIAG3, PATTERN_CROSS2, PATTERN_DIAG2, PATTERN_DIAG4, PATTERN_CROSS3, PATTERN_CROSS4, PATTERN_STRIPE1];
        $markers = [MARK_DIAMOND, MARK_SQUARE, MARK_CIRCLE, MARK_UTRIANGLE, MARK_DTRIANGLE, MARK_FILLEDCIRCLE, MARK_CROSS, MARK_STAR, MARK_X];
